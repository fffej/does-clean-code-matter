...
public sealed class SliceApplicationTests
{
    ...
    [TestMethod]
    public async Task RunAsync_WithJsonFormat_RendersTabularResultsAsJsonObjects()
    {
        var tempFile = Path.Combine(Path.GetTempPath(), $"{Guid.NewGuid():N}.csv");
        var input = string.Join("\r\n", [
            "name,age,city",
        ...
        var expected = "[{\"age\":\"36\",\"name\":\"Ada\"},{\"age\":\"37\",\"name\":\"Lovelace, Ada\"}]";
        var inputBytes = new UTF8Encoding(encoderShouldEmitUTF8Identifier: false).GetBytes(input);
        var expectedBytes = new UTF8Encoding(encoderShouldEmitUTF8Identifier: false).GetBytes(expected);
        ...
        try
        {
            ...
            var exitCode = await app.RunAsync([tempFile, "--format", "json", "select", "age,name"]);
    ...
    [TestMethod]
    public async Task RunAsync_WithTableFormat_RendersTabularResultsAsATable()
    {
        var tempFile = Path.Combine(Path.GetTempPath(), $"{Guid.NewGuid():N}.csv");
        var input = string.Join("\r\n", [
            "name,age,city",
        ...
        try
        {
            ...
            var exitCode = await app.RunAsync([tempFile, "select", "age,name", "--format", "table"]);
    ...
    [TestMethod]
    public async Task RunAsync_WithJsonFormat_RendersScalarResultsAsASingleJsonValue()
    {
        var tempFile = Path.Combine(Path.GetTempPath(), $"{Guid.NewGuid():N}.csv");
        var input = string.Join("\r\n", [
            "name,age,city",
        ...
        try
        {
            ...
            var exitCode = await app.RunAsync([tempFile, "count", "--format", "json"]);
